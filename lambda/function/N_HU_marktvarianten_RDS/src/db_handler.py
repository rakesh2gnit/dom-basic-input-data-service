...
def get_column_template():
    ...
    try:
        ...
        logger.error(f"Error fetching column templates: {e}")
        raise DatabaseException(f"Error fetching column templates. Please connect with dev team.")
...
def load_drive_template():
    ...
    try:
        ...
        logger.error(f"Error loading drive template: {e}")
        raise DatabaseException(f"Error loading drive template. Please connect with dev team.")
...
def get_existing_file_name():
    ...
    try:
        ...
        logger.error(f"Error fetching existing file name: {e}")
        raise DatabaseException(f"Error fetching existing file name. Please contact dev team.")
...
def load_drive_content(file_name, file_path):
    ...
    try:
        ...
        raise DatabaseException(f"Error loading drive content. Please connect with dev team.")
...
def get_drive_table_data(columns_to_insert):
    ...
    try:
        ...
        logger.error(f"Error fetching {Constants.DRIVE_NAME} data: {e}")
        raise DatabaseException(f"Error fetching {Constants.DRIVE_NAME} data. Please connect with dev team.")
...
def update_drive_table_data(df_database, df_to_update, column_to_insert):
    for index, row in df_to_update.iterrows():
        ...
        try:
            ...
            logger.error(f"Error updating row: {e}, {row['key']}")
            raise DatabaseException(f"Error updating SA tank data. Please connect with dev team.")
        ...
        try:
            ...
            with db.get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    conn.commit()
                    logger.info("inserted in database: ", index, row['key'])
                    db.release_connection(conn)
        except Exception as e:
            logger.error(f"Error inserting row: {e}, {row['key']}")
            raise DatabaseException(f"Error inserting SA tank data. Please connect with dev team.")
...
def insert_data_log_api(data):
    ...
    try:
        ...
        logger.error(f"Error inserting data log table: {e}")
        raise DatabaseException(f"Error inserting data log table. Please connect with dev team.")
